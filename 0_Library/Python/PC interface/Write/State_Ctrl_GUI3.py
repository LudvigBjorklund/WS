...
class StateMachineGUI(QMainWindow):
    ...
    def initUI(self):
        ...
        preview_button = QPushButton("Preview Converted Value")
        preview_button.clicked.connect(self.preview_custom_data)
        main_layout.addWidget(preview_button)  # Add it at an appropriate position in your layout
    ...
    def preview_custom_data(self):
        try:
            ...
            # and convert the value using the user provided format (or default to "24EN0")
            ...
            # Ensure the binary string is exactly 32 bits (pad or trim accordingly)
            if len(combined_bin) < 32:
                combined_bin = combined_bin.rjust(32, '0')
            elif len(combined_bin) > 32:
                combined_bin = combined_bin[-32:]
            
            # Convert combined binary string to integer
            combined_data = int(combined_bin, 2)
            
            # Display the integer value in a message box
            QMessageBox.information(self, "Converted Value",
                                    f"Converted value (integer): {combined_data}")
        except Exception as e:
            QMessageBox.warning(self, "Error", f"Conversion failed: {e}")
    def send_custom_data(self):
        try:
            # Read decimal values from the UI
            id_decimal = float(self.id_input.text())
            value_decimal = float(self.value_input.text())

            # Convert the ID using fixed format "8EN0"
            id_bin = preprocess.decimal_to_binary_string(id_decimal, "8EN0")
            # For the value, either use the provided format or default to "24EN0"
            fmt_str = self.value_fmt_input.text().strip() or "24EN0"
            value_bin = preprocess.decimal_to_binary_string(value_decimal, fmt_str)

            # Combine the binary strings: ID (8 bits) + Value (should be 24 bits)
            combined_bin = id_bin + value_bin
            ...
            if len(combined_bin) < 32:
                ...
                # Option: trim the extra bits; adjust if you prefer to error instead.
                combined_bin = combined_bin[-32:]

            # Convert binary string to integer and send to FPGA
            combined_data = int(combined_bin, 2)
